...
class LocalSemanticIndex:
    ...
    def _load_model(self):
        if self.model is None:
            try:
                ...
                raise RuntimeError(
                    "Install local semantic dependencies first: "
                    "pip install sentence-transformers numpy"
                ) from exc
            self.model = SentenceTransformer(self.model_name)
        return self.model

    def build(self, records: list, batch_size: int = 64):

        import numpy as np

        model = self._load_model()
        self.records = list(records)
        texts = [semantic_text(r) for r in self.records]
    ...
    def save(self, directory: str):
        ...
        with open(path / "metadata.json", "w", encoding="utf-8") as f:
            json.dump({"model_name": self.model_name, "count": len(self.records)}, f)
    ...
    def search(self, query: dict, candidate_limit: int = 200) -> list:
        ...
        if self.embeddings is None or not self.records:
            raise RuntimeError("Semantic index is empty; call build() or load()")
        ...
        allowed = [
            ...
            if passes_hard_filters(query, record)
        ]
        if not allowed:
            return []

        matrix = self.embeddings[allowed]
        similarities = matrix @ q
        k = min(candidate_limit, len(allowed))
        
        if k < len(allowed):
            top_local = np.argpartition(-similarities, k - 1)[:k]
        else:
            top_local = np.arange(len(allowed))
        top_local = top_local[np.argsort(-similarities[top_local])]

        results = []
        for local_idx in top_local:
            original_idx = allowed[int(local_idx)]
            record = dict(self.records[original_idx])
            record["semantic_similarity"] = round(float(similarities[local_idx]), 4)
            results.append(record)
        return results


def rank_candidates_local(
    missing: dict,
    index: LocalSemanticIndex,
    candidate_limit: int = 200,
    top_n_expensive: int = 20,
    face_compare_fn=None,
) -> list:

    candidates = index.search(missing, candidate_limit=candidate_limit)
    ...
    results = rank_candidates(
        ...
        face_compare_fn=face_compare_fn,
    )
    ...
    for result in results:
        ...
        result["semantic_similarity"] = by_id.get(cid)
